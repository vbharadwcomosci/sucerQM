...
def GetZ(ringsize, q, ang):
    ...
    qsize = len(q)
    angsize = len(ang)
    assert ((qsize+angsize)==(ringsize-3)) and ringsize>4 and ringsize<16, "Inappropriate Input"
    Z = []
...
def GetRingSubstituentPosition(mol, ring, ring_substituent):
    ...
    alpha = np.ndarray.item(np.arccos(fixzero(np.dot(s,n))))
    ...
    beta = np.ndarray.item(np.arctan2(-sv,su))
    return alpha, beta
